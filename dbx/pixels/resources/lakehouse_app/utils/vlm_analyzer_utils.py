...
def normalize_image(image_array: np.ndarray) -> np.ndarray:
    ...
    if image_array.dtype != np.uint8:
        ...
        if image_max > image_min:
            normalized = ((image_array - image_min) / (image_max - image_min) * 255).astype(
                np.uint8
            )
        else:
...
def convert_to_base64(image_array: np.ndarray, format: str = "JPEG", quality: int = 85) -> str:
    ...
    print(
        f"Image converted to base64 ({format} format, quality={quality if format.upper()=='JPEG' else 'N/A'})"
    )
