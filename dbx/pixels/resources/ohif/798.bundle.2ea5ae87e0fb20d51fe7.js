...
function utils_processResults(qidoStudies) {
  ...
  qidoStudies.forEach(qidoStudy => studies.push({
    ...
    date: utils_getString(JSON.parse(qidoStudy[1])),
    // YYYYMMDD
    time: utils_getString(JSON.parse(qidoStudy[2])),
    // HHmmss.SSS (24-hour, minutes, seconds, fractional seconds)
    accession: utils_getString(JSON.parse(qidoStudy[3])) || '',
    // short string, probably a number?
    mrn: utils_getString(JSON.parse(qidoStudy[4])) || '',
    // medicalRecordNumber
