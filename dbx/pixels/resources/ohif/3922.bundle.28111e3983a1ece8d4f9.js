(self["webpackChunk"] = self["webpackChunk"] || []).push([[3922],{
...
;// CONCATENATED MODULE: ../../../../../../../plugins/ohifv3/extensions/default/src/DatabricksPixelsDicom/utils.js
...
// EXTERNAL MODULE: ../../../../../../../plugins/ohifv3/extensions/default/src/DatabricksPixelsDicom/StaticWadoClient.ts + 2 modules
var StaticWadoClient = __webpack_require__(39062);
...
;// CONCATENATED MODULE: ../../../../../../../plugins/ohifv3/extensions/default/src/DatabricksPixelsDicom/multiframeImageLoader.js
/**
 * Simplified Multiframe DICOM Progressive Loader with Multi-Threading
 * 
 * Strategy:
 * 1. Fetch metadata separately (lightweight)
 * 2. Stream the full file in background
 * 3. Decode frames as soon as their data is available
 * 4. Leverage web workers for parallel decoding (configured via dicomImageLoader)
 * 5. Use GPU acceleration when available (configured via Cornerstone)
 * 
 * Performance optimizations:
 * - Web workers handle JPEG/JPEG-LS/RLE decoding in parallel
 * - GPU rendering via WebGL for viewport display
 * - Shared cache across all frames from the same file
 */
...
function lowercaseFirstLetterOfKeys(obj) {
  if (Array.isArray(obj)) {
    return obj.map(lowercaseFirstLetterOfKeys);
  }
  if (obj !== null && typeof obj === 'object') {
    return Object.keys(obj).reduce((result, key) => {
      const newKey = key.charAt(0).toLowerCase() + key.slice(1);
      result[newKey] = lowercaseFirstLetterOfKeys(obj[key]);
      return result;
    }, {});
  }
  return obj;
...
class MultiframeImageLoaderService {
  constructor() {
    ...
    this.metadataCache = new Map(); // Cache for metadata
    this.streamingBuffers = new Map(); // Active streams with combined buffers
    this.loadedFrames = new Map(); // Track loaded frames per file for progress logging

    // Track concurrent decode operations for performance monitoring
    this.decodeStats = {
      concurrent: 0,
      maxConcurrent: 0,
      totalDecoded: 0,
      totalDecodeTime: 0,
      // Cumulative decode time in ms
      fileStartTime: null,
      // When first frame decode started
      fileDecodeStarted: new Map() // Track decode start time per file
    };
  }
  initialize() {
    const cpuCores = typeof navigator !== 'undefined' ? navigator.hardwareConcurrency : 'unknown';
    const gpuEnabled = typeof esm !== 'undefined' ? !esm.getShouldUseCPURendering() : 'unknown';
    console.log(`[MULTIFRAME] Progressive multiframe loader initialized\n` + `  • CPU Cores: ${cpuCores}\n` + `  • Web Workers: Enabled (parallel JPEG/JPEG-LS/RLE decoding)\n` + `  • GPU Acceleration: ${gpuEnabled ? 'Enabled (WebGL rendering)' : 'Disabled (CPU fallback)'}\n` + `  • Prefetch Pool: 20 concurrent requests (configurable via maxNumRequests.prefetch)`);
  }

  /**
   * Main entry point - load a frame from multiframe DICOM
   */
  async loadImage(imageId) {
    const [fileUrl, frameNumberStr] = imageId.split('&frame=');
    const cleanUrl = fileUrl.replace('multiframe:', '');
    const frameNumber = parseInt(frameNumberStr, 10);
    try {
      // Step 1: Get metadata (fast, cached after first request)
      const metadata = await this.getMetadata(cleanUrl);

      // Create shared cache key for all frames from this file
      const sharedCacheKey = `multiframe:${cleanUrl}`;

      // Register metadata provider for Cornerstone
      this.registerMetadataProvider(fileUrl, metadata);

      // Step 2: Start streaming file if not already (non-blocking)
      this.ensureStreamingStarted(cleanUrl);

      // Step 3: Extract only the bytes needed for this specific frame
      const framePixelData = await this.extractFrameBytes(cleanUrl, frameNumber, metadata);
      const zeroBasedFrameIndex = frameNumber - 1;
      const transferSyntax = metadata.transferSyntaxUID || '1.2.840.10008.1.2';

      // Prepare pixel data as Uint8Array for decoding
      const pixelDataAsUint8Array = new Uint8Array(framePixelData);
      ...
      // Decode options - Web workers are automatically used by decodeImageFrame
      // for JPEG, JPEG-LS, and RLE transfer syntaxes
      const decodeOptions = {
        preScale: preScale
      };

      // Track concurrent operations for performance monitoring
      this.decodeStats.concurrent++;
      this.decodeStats.maxConcurrent = Math.max(this.decodeStats.maxConcurrent, this.decodeStats.concurrent);

      // Mark when first decode starts for this file
      if (!this.decodeStats.fileDecodeStarted.has(cleanUrl)) {
        this.decodeStats.fileDecodeStarted.set(cleanUrl, Date.now());
      }

      // Log when we hit high concurrency (indicates parallel loading is working)
      if (this.decodeStats.concurrent > 1 && this.decodeStats.concurrent === this.decodeStats.maxConcurrent) {
        console.log(`[MULTIFRAME] 🚀 Peak concurrency: ${this.decodeStats.concurrent} frames decoding in parallel!`);
      }
      const decodeStartTime = performance.now();
      try {
        // decodeImageFrame automatically uses web workers for compressed data
        // The number of workers is configured via dicomImageLoader.init({ maxWebWorkers })
        const decodedImageFrame = await dist_esm/* decodeImageFrame */.pM(metadata, transferSyntax, pixelDataAsUint8Array, undefined, decodeOptions);
        const decodeEndTime = performance.now();
        const decodeTime = decodeEndTime - decodeStartTime;
        this.decodeStats.totalDecoded++;
        this.decodeStats.totalDecodeTime += decodeTime;
        return await this.buildCornerstoneImage(decodedImageFrame, metadata, imageId, cleanUrl, frameNumber, sharedCacheKey, decodeTime);
      } finally {
        this.decodeStats.concurrent--;
      }
    } catch (error) {
      console.error(`[MULTIFRAME] Error loading frame ${frameNumber}:`, error);
      throw error;
    }
  }

  /**
   * Build the Cornerstone image object from decoded frame
   */
  async buildCornerstoneImage(decodedImageFrame, metadata, imageId, cleanUrl, frameNumber, sharedCacheKey, decodeTime) {
    try {
      // Validate decoded frame
      if (!decodedImageFrame) {
        throw new Error(`Failed to decode frame ${frameNumber}`);
      }
      const pixelData = decodedImageFrame.pixelData || decodedImageFrame;
      if (!pixelData || !pixelData.length && !pixelData.byteLength) {
        throw new Error(`Failed to decode frame ${frameNumber} - no pixel data`);
      }

      // Build Cornerstone image object with essential properties
      ...
      const image = {
        ...
        columnPixelSpacing: metadata.columnPixelSpacing || 1,
        rowPixelSpacing: metadata.rowPixelSpacing || 1,
        invert: metadata.photometricInterpretation === 'MONOCHROME1',
        getPixelData: () => pixelData,
        minPixelValue: decodedImageFrame.minPixelValue || 0,
        maxPixelValue: decodedImageFrame.maxPixelValue || 65535,
        slope: metadata.rescaleSlope || 1,
        intercept: metadata.rescaleIntercept || 0,
        windowCenter: metadata.windowCenter,
        windowWidth: metadata.windowWidth,
        sizeInBytes: metadata.rows * metadata.columns * bytesPerPixel * samplesPerPixel,
        imageFrame: {
          pixelData
        },
        sharedCacheKey: sharedCacheKey,
        imageQualityStatus: esm.Enums.ImageQualityStatus.FULL_RESOLUTION
      };

      // Track progress and log milestones
      if (!this.loadedFrames.has(cleanUrl)) {
        this.loadedFrames.set(cleanUrl, new Set());
        console.log(`[MULTIFRAME] ⏱️  Started decoding: ${cleanUrl.split('/').pop()} ` + `(${metadata.numberOfFrames || 'unknown'} frames) at ${new Date().toLocaleTimeString()}`);
      }
      const loadedSet = this.loadedFrames.get(cleanUrl);
      loadedSet.add(frameNumber);
      const totalFrames = metadata.numberOfFrames || 0;
      const loadedCount = loadedSet.size;

      // Log every 10th frame or the last frame with timing info
      if (frameNumber % 10 === 0 || loadedCount === totalFrames) {
        const avgDecodeTime = this.decodeStats.totalDecodeTime / this.decodeStats.totalDecoded;
        console.log(`[MULTIFRAME] Progress: ${loadedCount}/${totalFrames} frames decoded ` + `(${(loadedCount / totalFrames * 100).toFixed(1)}%) | ` + `Avg: ${avgDecodeTime.toFixed(1)}ms/frame | ` + `This frame: ${decodeTime.toFixed(1)}ms`);
      }

      // Log completion with comprehensive performance stats
      if (loadedCount === totalFrames) {
        const fileDecodeStartTime = this.decodeStats.fileDecodeStarted.get(cleanUrl);
        const totalElapsedTime = Date.now() - fileDecodeStartTime;
        const avgDecodeTime = this.decodeStats.totalDecodeTime / this.decodeStats.totalDecoded;
        const throughput = totalFrames / (totalElapsedTime / 1000);
        console.log(`[MULTIFRAME] ✓ Decoding complete: ${cleanUrl.split('/').pop()} at ${new Date().toLocaleTimeString()}\n` + `  • Total frames: ${totalFrames}\n` + `  • Total elapsed time: ${(totalElapsedTime / 1000).toFixed(2)}s\n` + `  • Total decode time: ${(this.decodeStats.totalDecodeTime / 1000).toFixed(2)}s\n` + `  • Average decode time: ${avgDecodeTime.toFixed(1)}ms/frame\n` + `  • Max concurrent: ${this.decodeStats.maxConcurrent} workers\n` + `  • Throughput: ${throughput.toFixed(1)} frames/sec`);

        // Clean up
        this.loadedFrames.delete(cleanUrl);
        this.decodeStats.fileDecodeStarted.delete(cleanUrl);

        // Reset stats for next file
        this.decodeStats.maxConcurrent = 0;
        this.decodeStats.totalDecodeTime = 0;
        this.decodeStats.totalDecoded = 0;
      }
      return image;
    } catch (error) {
      console.error(`[MULTIFRAME] Error building image for frame ${frameNumber}:`, error);
      throw error;
    }
  }

  /**
   * Extract only the bytes needed for a specific frame (memory efficient)
   */
  async extractFrameBytes(fileUrl, frameNumber, metadata) {
    // Get streaming data
    const streamState = this.streamingBuffers.get(fileUrl);
    if (!streamState) {
      throw new Error('Stream not started');
    }

    // Wait for enough data for this frame
    await this.waitForFrameData(streamState, frameNumber, metadata);

    // Use combined buffer if available (stream complete), otherwise combine temporarily
    let buffer;
    if (streamState.combinedBuffer) {
      // Stream complete, use cached combined buffer
      buffer = streamState.combinedBuffer;
    } else {
      // Stream still active, combine chunks temporarily for this frame
      buffer = this.combineChunks(streamState.chunks, streamState.bytesReceived);
      // This temporary buffer will be garbage collected after extraction
    }

    // Extract and return only this frame's bytes
    return this.extractFrameFromBuffer(buffer, frameNumber, metadata);
  }

  /**
   * Extract frame bytes from a buffer
   */
  extractFrameFromBuffer(byteArray, frameNumber, metadata) {
    const zeroBasedFrameIndex = frameNumber - 1;
    const transferSyntax = metadata.transferSyntaxUID || '1.2.840.10008.1.2';
    const isCompressed = transferSyntax !== '1.2.840.10008.1.2' && transferSyntax !== '1.2.840.10008.1.2.1' && transferSyntax !== '1.2.840.10008.1.2.2';
    if (isCompressed) {
      // For compressed data, extract the specific frame
      const pixelDataOffset = metadata.pixelDataOffset || this.findPixelDataOffset(byteArray);
      const fragments = this.parseFragments(byteArray, pixelDataOffset);
      if (zeroBasedFrameIndex >= fragments.length) {
        throw new Error(`Frame ${frameNumber} not found (only ${fragments.length} fragments)`);
      }
      return fragments[zeroBasedFrameIndex];
    } else {
      // For uncompressed data, calculate frame offset
      const frameSize = metadata.rows * metadata.columns * ((metadata.bitsAllocated || 16) / 8) * (metadata.samplesPerPixel || 1);
      const pixelDataOffset = metadata.pixelDataOffset || this.findPixelDataOffset(byteArray);
      const frameOffset = pixelDataOffset + frameSize * zeroBasedFrameIndex;
      if (frameOffset + frameSize > byteArray.length) {
        throw new Error(`Frame ${frameNumber} exceeds buffer size`);
      }
      return byteArray.slice(frameOffset, frameOffset + frameSize);
    }
  }

  /**
   * Find pixel data offset by scanning for (7FE0,0010) tag
   * Does NOT parse the full DICOM - just scans for the tag
   */
  findPixelDataOffset(byteArray) {
    // Skip DICOM preamble and DICM marker if present
    let searchStart = 0;
    if (byteArray.length > 132 && byteArray[128] === 0x44 && byteArray[129] === 0x49 && byteArray[130] === 0x43 && byteArray[131] === 0x4D) {
      searchStart = 132; // Skip preamble + DICM
    }

    // Scan for pixel data tag (7FE0,0010)
    const view = new DataView(byteArray.buffer, byteArray.byteOffset);
    for (let i = searchStart; i < byteArray.length - 8; i++) {
      const group = view.getUint16(i, true);
      const element = view.getUint16(i + 2, true);
      if (group === 0x7fe0 && element === 0x0010) {
        // Found pixel data tag, VR is next 2 bytes
        const vr = String.fromCharCode(byteArray[i + 4], byteArray[i + 5]);
        if (vr === 'OB' || vr === 'OW') {
          // Explicit VR: tag(4) + VR(2) + reserved(2) + length(4) = 12 bytes header
          return i + 12;
        } else {
    ...
    throw new Error('Pixel Data tag (7FE0,0010) not found in buffer');
  }

  /**
   * Parse compressed pixel data fragments
   */
  parseFragments(byteArray, pixelDataOffset) {
    const fragments = [];
    let offset = pixelDataOffset;

    // Read first item (Basic Offset Table) - skip it
    const view = new DataView(byteArray.buffer, byteArray.byteOffset);
    let group = view.getUint16(offset, true);
    let element = view.getUint16(offset + 2, true);
    let length = view.getUint32(offset + 4, true);
    if (group === 0xfffe && element === 0xe000) {
      offset += 8 + length; // Skip BOT
    }

    // Read all fragments
    while (offset < byteArray.length - 8) {
      group = view.getUint16(offset, true);
      ...
      if (group === 0xfffe && element === 0xe000) {
        ...
        const fragmentStart = offset + 8;
        const fragmentEnd = fragmentStart + length;
        if (fragmentEnd <= byteArray.length) {
          fragments.push(byteArray.slice(fragmentStart, fragmentEnd));
          offset = fragmentEnd;
        } else {
          break;
        }
   ...
   */
  registerMetadataProvider(fileUrl, metadata) {
    // Only register once
    if (this.metadataCache.has(fileUrl + '_registered')) {
      return;
    }
    ...
    this.metadataCache.set(fileUrl + '_registered', true);
  }

  /**
   * Fetch and cache metadata from Databricks metadata endpoint
   */
  async getMetadata(fileUrl) {
    if (this.metadataCache.has(fileUrl)) {
      return this.metadataCache.get(fileUrl);
    }
    const metadataUrl = fileUrl.replace("/files/", "/files_metadata/");
    const response = await fetch(metadataUrl);
    if (!response.ok) {
      throw new Error(`Failed to fetch metadata: ${response.status}`);
    }
    const rawMetadata = await response.json();
    const metadata = lowercaseFirstLetterOfKeys(naturalizeDataset(rawMetadata));
    this.metadataCache.set(fileUrl, metadata);
    return metadata;
  }

  /**
   * Start streaming the file if not already streaming
   */
  ensureStreamingStarted(fileUrl) {
    if (this.streamingBuffers.has(fileUrl)) {
      return;
    }
    const streamState = {
      chunks: [],
    ...
    this.streamingBuffers.set(fileUrl, streamState);

    // Start streaming in background (non-blocking)
    this.streamFile(fileUrl, streamState).catch(error => {
      console.error('[MULTIFRAME] Stream error:', error);
   ...
   */
  async streamFile(fileUrl, streamState) {
    ...
    try {
      ...
      console.log(`[MULTIFRAME] Streaming started: ${fileName} (${(contentLength / 1024 / 1024).toFixed(2)} MB)`);
      while (true) {
        const {
          done,
          value
        ...
        if (done) {
          ...
          const duration = (Date.now() - streamState.startTime) / 1000;

          // Combine chunks into single buffer for efficient frame extraction
          if (!streamState.combinedBuffer) {
            streamState.combinedBuffer = this.combineChunks(streamState.chunks, streamState.bytesReceived);
            streamState.chunks = null; // Free memory
          }
          console.log(`[MULTIFRAME] ✓ Streaming complete: ${fileName} (${(streamState.bytesReceived / 1024 / 1024).toFixed(2)} MB in ${duration.toFixed(1)}s)`);
          break;
        }

        // Store chunk (no reallocation needed!)
        if (streamState.chunks) {
          streamState.chunks.push(value);
        }
        streamState.bytesReceived += value.length;
      }
    } catch (error) {
      streamState.error = error;
      throw error;
  ...
  /**
   * Wait for enough data to decode a frame
   * Uses offset table for compressed data, byte calculation for uncompressed
   */
  async waitForFrameData(streamState, frameNumber, metadata) {
    // Check if compressed
    const transferSyntax = metadata.transferSyntaxUID || '1.2.840.10008.1.2';
    const isCompressed = transferSyntax !== '1.2.840.10008.1.2' && transferSyntax !== '1.2.840.10008.1.2.1' && transferSyntax !== '1.2.840.10008.1.2.2';
    ...
    return new Promise((resolve, reject) => {
      ...
      const checkInterval = setInterval(() => {
        ...
        let estimatedBytesNeeded = 0;
        if (isCompressed) {
          // Try to parse offset table if we haven't yet
          if (!frameOffsets && streamState.bytesReceived > 100000) {
            // Wait for ~100KB header
            try {
              // Use combined buffer if available, otherwise combine chunks temporarily
              let buffer;
              if (streamState.combinedBuffer) {
                buffer = streamState.combinedBuffer;
              } else if (streamState.chunks && streamState.chunks.length > 0) {
                buffer = this.combineChunks(streamState.chunks, streamState.bytesReceived);
              } else {
                // Chunks cleared, skip
                buffer = null;
              ...
              if (buffer) {
                pixelDataOffset = metadata.pixelDataOffset || this.findPixelDataOffset(buffer);
                frameOffsets = this.parseFragmentOffsets(buffer, pixelDataOffset);
              }
            } catch (e) {
          ...
          // If we have offsets, check if we have the specific frame
          if (frameOffsets && frameOffsets.length > 0) {
            const zeroBasedIndex = frameNumber - 1;
            if (zeroBasedIndex < frameOffsets.length) {
              const frameStart = frameOffsets[zeroBasedIndex];
              // Estimate frame end (next frame start or add reasonable size)
              const frameEnd = zeroBasedIndex + 1 < frameOffsets.length ? frameOffsets[zeroBasedIndex + 1] : frameStart + 200000; // ~200KB estimate for last frame

              estimatedBytesNeeded = frameEnd;
              hasEnoughData = streamState.bytesReceived >= estimatedBytesNeeded || streamState.complete;
            }
          } else {
            // No offsets yet, wait for complete
            hasEnoughData = streamState.complete;
          }
        } else {
          // For uncompressed data, calculate exact offset
          const frameSize = metadata.rows * metadata.columns * ((metadata.bitsAllocated || 16) / 8) * (metadata.samplesPerPixel || 1);
          const frameOffset = frameSize * (frameNumber - 1);
          estimatedBytesNeeded = frameOffset + frameSize + 50000; // +50KB for header
      ...
      setTimeout(() => {
        clearInterval(checkInterval);
        reject(new Error(`Timeout waiting for frame ${frameNumber}`));
      }, 60000);
    });
  }

  /**
   * Parse fragment offsets from offset table (returns byte positions)
   */
  parseFragmentOffsets(byteArray, pixelDataOffset) {
    const offsets = [];
    let offset = pixelDataOffset;
    ...
    let length = view.getUint32(offset + 4, true);
    if (group === 0xfffe && element === 0xe000) {
      // BOT contains frame offsets relative to first frame
      const botStart = offset + 8;
      if (length > 0 && length < 100000) {
        // Sanity check
        const numOffsets = length / 4;
        for (let i = 0; i < numOffsets; i++) {
          const frameOffset = view.getUint32(botStart + i * 4, true);
          // Offsets are relative to first fragment, add BOT header size
          offsets.push(pixelDataOffset + 8 + length + frameOffset);
        }
      }
      offset += 8 + length;
    }

    // If no BOT or BOT is empty, scan fragments
    if (offsets.length === 0) {
      while (offset < byteArray.length - 8) {
        group = view.getUint16(offset, true);
        element = view.getUint16(offset + 2, true);
        ...
        if (group === 0xfffe && element === 0xe000) {
          offsets.push(offset + 8); // Frame data starts after item tag+length
          length = view.getUint32(offset + 4, true);
          offset += 8 + length;
        } else {
          break;
...
;// CONCATENATED MODULE: ../../../../../../../plugins/ohifv3/extensions/default/src/DatabricksPixelsDicom/index.js
...
function createDatabricksPixelsDicom(dcmConfig, servicesManager) {
  ...
  // Create a SINGLE SHARED instance of the multiframe loader
  // This ensures all frames share the same cache and stream state
  const multiframeLoader = new multiframeImageLoader();
  multiframeLoader.initialize();
  ...
  const implementation = {
    initialize: async ({
      ...
      // Register the SHARED loader instance (not a new one each time!)
      esm.registerImageLoader('multiframe', (imageId, options) => {
        const loadPromise = multiframeLoader.loadImage(imageId).then(image => {
          // Validate image before returning
          if (!image || !image.imageId) {
            console.error('[MULTIFRAME] Invalid image object returned:', image);
            throw new Error('Invalid image object - missing imageId');
          }
          if (!image.getPixelData || typeof image.getPixelData !== 'function') {
            console.error('[MULTIFRAME] Invalid image object - getPixelData is not a function');
            throw new Error('Invalid image object - missing getPixelData function');
          }
          return image;
        }).catch(error => {
          // Ensure error is properly formatted for Cornerstone
          console.error('[MULTIFRAME] Image load error:', error);
          const errorObj = {
        ...
        return {
          ...
          cancelFn: () => {
            ...
            console.log('[MULTIFRAME] Cancel requested for:', imageId);
          }
        };
      });
...
function calculateScanAxisNormal(imageOrientation) {
  const rowCosineVec = gl_matrix_esm/* vec3.fromValues */.eR.fromValues(imageOrientation[0], imageOrientation[1], imageOrientation[2]);
  const colCosineVec = gl_matrix_esm/* vec3.fromValues */.eR.fromValues(imageOrientation[3], imageOrientation[4], imageOrientation[5]);
...
function getDisplaySetsFromUnsupportedSeries(instances) {
  ...
  const messages = new src.DisplaySetMessageList();
  messages.addMessage(src.DisplaySetMessage.CODES.UNSUPPORTED_DISPLAYSET);
...
// EXTERNAL MODULE: ../../../node_modules/dicomweb-client/build/dicomweb-client.es.js
var dicomweb_client_es = __webpack_require__(83562);
;// CONCATENATED MODULE: ../../../../../../../plugins/ohifv3/extensions/default/src/DatabricksPixelsDicom/findIndexOfString.ts
function checkToken(token, data, dataOffset) {
  if (dataOffset + token.length > data.length) {
    return false;
  }
...
;// CONCATENATED MODULE: ../../../../../../../plugins/ohifv3/extensions/default/src/DatabricksPixelsDicom/fixMultipart.ts
...
;// CONCATENATED MODULE: ../../../../../../../plugins/ohifv3/extensions/default/src/DatabricksPixelsDicom/StaticWadoClient.ts


const {
  DicomMetadataStore
} = __webpack_require__(62037);
const {
  DICOMwebClient
} = dicomweb_client_es/* api */.FH;
const anyDicomwebClient = DICOMwebClient;
