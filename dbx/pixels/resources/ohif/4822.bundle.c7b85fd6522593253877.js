...
function determinePixelDataInfo(metadata, imageId) {
  ...
  const windowCenter = Array.isArray(WindowCenter) ? WindowCenter[0] : WindowCenter;
  const windowWidth = Array.isArray(WindowWidth) ? WindowWidth[0] : WindowWidth;
  ...
  async fetchFrame(fileUrl, frameNumber) {
    try {
      ...
      if (!metadata || !metadata.TransferSyntaxUID) {
        ...
        metadata = naturalizeDataset(rawMetadata);
