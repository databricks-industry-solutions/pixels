(self["webpackChunk"] = self["webpackChunk"] || []).push([[617],{
...
var sha1 = __webpack_require__(91430);
...
const {
  getString: utils_getString,
  getName: utils_getName,
  getModalities: utils_getModalities
} = src.DICOMWeb;
const SQL_STATEMENT_API = "sql/statements/";
function utils_processResults(qidoStudies) {
  if (!qidoStudies || !qidoStudies.length) {
    return [];
  }
  const studies = [];
  qidoStudies.forEach(qidoStudy => studies.push({
    studyInstanceUid: utils_getString(JSON.parse(qidoStudy[0])),
    date: qidoStudy[1] || '',
    // YYYYMMDD
    time: qidoStudy[2] || '',
    // HHmmss.SSS (24-hour, minutes, seconds, fractional seconds)
    ...
    mrn: qidoStudy[4] || '',
    // medicalRecordNumber
    patientName: qidoStudy[5].includes("Alphabetic") ? src.utils.formatPN(utils_getName(JSON.parse(qidoStudy[5]))) : "",
...
function utils_processSeriesResults(qidoSeries) {
  const series = [];
  if (qidoSeries && qidoSeries.length) {
    qidoSeries.forEach(qidoSeries => series.push({
      studyInstanceUid: utils_getString(JSON.parse(qidoSeries[0])),
      seriesInstanceUid: utils_getString(JSON.parse(qidoSeries[1])),
      modality: utils_getString(JSON.parse(qidoSeries[2])),
      seriesNumber: utils_getString(JSON.parse(qidoSeries[3])),
      description: utils_getString(JSON.parse(qidoSeries[4])),
 ...
 * @returns {Promise} - Resolves SeriesMetadata[] in study
 */
function utils_seriesInStudy(dicomWebClient, studyInstanceUID) {
  // Series Description
  // Already included?
  const commaSeparatedFields = ['0008103E', '00080021'].join(',');
  const queryParams = {
 ...
 * @returns {string} The URL with encoded filter query data
 */
function utils_mapParams(params) {
  let options = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
  if (!params) {
    return;
  }
...
const {
  ...
  DicomDict: DatabricksPixelsDicom_DicomDict
} = dcmjs_es["default"].data;
const {
...
function createDatabricksPixelsDicom(dcmConfig, servicesManager) {
  ...
  const implementation = {
    ...
    query: {
      studies: {
        mapParams: utils_mapParams.bind(),
        search: async function (origParams) {
          ...
          return utils_processResults(results);
        },
        processResults: utils_processResults.bind()
      },
      series: {
        search: async studyInstanceUID => {
          ...
          const result = utils_processSeriesResults(series);
          return result;
        }
      },
      instances: {
...
function ViewerHeader(_ref) {
  ...
  const versionNumber = "3.8.0-beta.0";
  const commitHash = "55002fbd712b6470b2217f75588e6a43366803e6";
  const menuOptions = [{
