...
class RedactionService {
  ...
  async exportRedactedDICOM(viewportId, viewport) {
    try {
      ...
      // If there are global redactions, include ALL files in the series
      // If there are NO global redactions, only include files with frame-specific redactions
      let filesToEdit;
      if (totalGlobalRedactions > 0) {
        filesToEdit = Array.from(allFilesInSeries.values()).map(file => file.filePath);
        console.log(`filesToEdit contains ${filesToEdit.length} files (complete series - has global redactions)`);
      } else {
        filesToEdit = Object.keys(files);
        console.log(`filesToEdit contains ${filesToEdit.length} files (only files with frame-specific redactions)`);
      }
      console.log(`files dictionary contains ${Object.keys(files).length} files (only those with frame-specific redactions)`);
