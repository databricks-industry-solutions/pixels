...
class PresidioTransformer(Transformer):
    ...
    def __init__(
        self, inputCol="path", outputCol="pii_analysis", padding_width=25, use_metadata=True
    ):
    ...
    def safe_slice(self, arr, index, property):
        return (
            arr[index][property]
            if (0 <= index < len(arr) and arr[index].get(property) is not None)
            else None
        )
    ...
    def _transform(self, df):
        ...
        def presidio_verify_udf(paths: pd.Series) -> pd.Series:
            ...
            def detect_phi(dcm_path: str):
                ...
                verification_image, ocr_results, analyzer_results = analyzer.verify_dicom_instance(
                    instance, self.padding_width, use_metadata=self.use_metadata
                )
                    ...
                    results.append(
                        {
                            ...
                            "conf": float(r["conf"]),
                            "analyzer_entity_type": self.safe_slice(
                                analyzer_results, i, "entity_type"
                            ),
