...
class DatabricksM2MAuth:
    ...
    def __init__(self,
        ...
        if not principal_name:
            if client_id is None:
                try:
                    self.logger.info(f"Getting client ID from secret scope '{self.secrets_scope_name}' using default secrets_client_id_key '{self.secrets_client_id_key}'")
                    client_id = b64decode(self.workspace_client.secrets.get_secret(scope=self.secrets_scope_name, key=self.secrets_client_id_key).value).decode('utf-8')
            ...
            if client_secret is None:
                try: 
                    self.logger.info(f"Getting client secret from secret scope '{self.secrets_scope_name}' using default secrets_client_secret_key '{self.secrets_client_secret_key}'")
                    self.client_secret = b64decode(self.workspace_client.secrets.get_secret(scope=self.secrets_scope_name, key=self.secrets_client_secret_key).value).decode('utf-8')
                except Exception as e:
    ...
    def create_scope_if_not_exists(self):
        ...
        if self.secrets_scope_name not in [scope.name for scope in self.workspace_client.secrets.list_scopes()]:
    ...
    def _get_or_create_pixels_sp(self, client_id=None):
        ...
        if not service_principal:
            for sp in self.workspace_client.service_principals.list(filter=f"(DisplayName eq '{self.principal_name}')"):
                if sp.display_name == self.principal_name:
                    service_principal = sp
                    break

        if not service_principal:
            service_principal = self.workspace_client.service_principals.create(display_name=self.principal_name)
        ...
        self.workspace_client.secrets.put_secret(scope=self.secrets_scope_name, key=self.secrets_client_id_key, string_value=self.client_id)
        self.workspace_client.secrets.put_secret(scope=self.secrets_scope_name, key=self.secrets_client_app_id_key, string_value=self.client_app_id)

        self.logger.info(f"Using service principal {service_principal.display_name} with ID {service_principal.id}")
    ...
    def _create_service_principal_secret(self, account_api_token: str):
        ...
        self.workspace_client.secrets.put_secret(scope=self.secrets_scope_name, key=self.secrets_client_secret_key, string_value=self.client_secret)

        self.logger.info(f"Created service principal secret for '{self.principal_name}' with ID {self.service_principal.id} and placed in '{self.secrets_client_secret_key}' db secret.")
    ...
    def set_client_secret(self, client_secret):
        self.client_secret = client_secret
        self.workspace_client.secrets.put_secret(scope=self.secrets_scope_name, key=self.secrets_client_secret_key, string_value=self.client_secret)
        self.logger.info(f"Saved service principal secret and placed in '{self.secrets_client_secret_key}' db secret.")
    ...
    def refresh_token(self) -> str:
        ...
        if not self.client_secret and not self.account_api_token:
            raise ValueError("account_api_token must be provided to create a new service principal secret and generate m2m tokens.")
        ...
        if not self.client_id or not self.client_secret:
            raise ValueError("client_id and client_secret must be provided, add principal_name to create a new service principal.")
        ...
        if not self._is_token_valid():
            ...
            self.workspace_client.secrets.put_secret(scope=self.secrets_scope_name, key=self.secrets_access_token_key, string_value=self._access_token)
            self.logger.info(f"Saved service principal token and placed in '{self.secrets_access_token_key}' db secret.")
    
    def grant_permissions(self, table, volume):

        #Grant USE CATALOG permissions on CATALOG
        self.workspace_client.grants.update(full_name=table.split(".")[0],
            securable_type=catalog.SecurableType.CATALOG,
            changes=[
            catalog.PermissionsChange(
                add=[catalog.Privilege.USE_CATALOG],
                principal=self.client_app_id
            )
            ]
        )

        #Grant USE SCHEMA permissions on SCHEMA
        self.workspace_client.grants.update(full_name=table.split(".")[0]+"."+table.split(".")[1],
        ...
        self.workspace_client.grants.update(full_name=table,
        ...
        self.workspace_client.grants.update(full_name=volume,
        ...
        self.logger.info("Successfully granted All privileges on table and volume to service principal")
