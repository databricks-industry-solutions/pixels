"use strict";
(globalThis["rspackChunk"] = globalThis["rspackChunk"] || []).push([[3145], {
23568(__unused_rspack_module, __webpack_exports__, __webpack_require__) {
...
;// CONCATENATED MODULE: ../../../extensions/image-redactor/package.json
var package_namespaceObject = JSON.parse('{"UU":"@ohif/extension-image-redactor"}')
;// CONCATENATED MODULE: ../../../extensions/image-redactor/src/id.js
/*
...
;// CONCATENATED MODULE: ../../../extensions/image-redactor/src/tools/RedactionRectangleTool.ts
/*
OHIF Image Redactor Extension Copyright (2025) Databricks, Inc.
Author: Emanuele Rinaldi <emanuele.rinaldi@databricks.com>
...
;// CONCATENATED MODULE: ../../../extensions/image-redactor/src/models/RedactionArea.js
/*
OHIF Image Redactor Extension Copyright (2025) Databricks, Inc.
Author: Emanuele Rinaldi <emanuele.rinaldi@databricks.com>
...
class RedactionArea {
  ...
  extractDICOMMetadata(viewport) {
    ...
    // Fallback for DICOMweb URLs that don't contain real filenames
    // (e.g., wadouri: URLs where split('/').pop() gives "wado" instead of a .dcm file)
    if (!this.filePath && this.sopInstanceUID !== 'unknown') {
      // fileName: use SOP Instance UID when URL parsing didn't yield a real filename
      if (this.fileName === 'unknown.dcm' || !this.fileName.endsWith('.dcm')) {
        this.fileName = `${this.sopInstanceUID}.dcm`;
      }
      // filePath stays null — will be resolved at export time via resolveVolumePaths() API
    }
...
;// CONCATENATED MODULE: ../../../extensions/image-redactor/src/services/RedactionService.js
/*
OHIF Image Redactor Extension Copyright (2025) Databricks, Inc.
Author: Emanuele Rinaldi <emanuele.rinaldi@databricks.com>
...
class RedactionService {
   ...
   * @param {object} viewport - The viewport object
   */
  exportRedactionAreas(viewportId, viewport, resolvedPaths = {}) {
    ...
    allAreas.forEach(area => {
      ...
      if (!fileGroups.has(fileKey)) {
        // Use resolved path from API if area.filePath is missing (DICOMweb case)
        const filePath = area.filePath || resolvedPaths[area.sopInstanceUID] || null;
  ...
  /**
   * Resolve Volume paths for DICOM instances via backend API.
   * Called when using DICOMweb data source where imageIds don't contain /Volumes/ paths.
   * 
   * API contract:
   *   POST {serverHostname}/api/dicomweb/resolve_paths
   *   Request:  { studyInstanceUID: string, seriesInstanceUID: string }
   *   Response: { paths: { [sopInstanceUID]: "/Volumes/catalog/schema/volume/path/file.dcm" } }
   *
   * @param {string} studyInstanceUID
   * @param {string} seriesInstanceUID
   * @returns {Object} Map of sopInstanceUID → volumePath, or empty object on failure
   */
  async resolveVolumePaths(studyInstanceUID, seriesInstanceUID) {
    try {
      const config = window.config || {};
      const wadoRoot = config.dataSources?.find(ds => ds.namespace?.includes('dicomweb'))?.configuration?.wadoRoot || '';
      if (!wadoRoot) {
        console.warn('resolveVolumePaths: wadoRoot not found in config');
        return {};
      }
      const endpointUrl = `${wadoRoot}/resolve_paths`;
      console.log(`Resolving volume paths for study=${studyInstanceUID}, series=${seriesInstanceUID}`);
      ...
      if (!response.ok) {
        console.warn(`resolveVolumePaths: API returned ${response.status} ${response.statusText}`);
        return {};
      }
      const result = await response.json();
      // Expected: { paths: { "1.2.3.4": "/Volumes/catalog/schema/volume/path/file.dcm", ... } }
      return result.paths || {};
    } catch (error) {
      console.warn('resolveVolumePaths: failed to resolve volume paths:', error);
      return {};
    }
  }
  exportMetadata(viewport, resolvedPaths = {}) {
    ...
    for (const imageId of allImageIds) {
      try {
        ...
        if (metadata) {
          ...
          // Try Volumes path from imageId (works for databricksPixelsDicom data source)
          const match = imageId.match(/\/Volumes\/[^?]+/);
          if (match) {
            filePath = match[0].split("&")[0];
          } else {
            // DICOMweb fallback: use resolved path from backend API
            const sopUID = metadata.SOPInstanceUID;
            if (sopUID && resolvedPaths[sopUID]) {
              filePath = resolvedPaths[sopUID];
            }
          }
  ...
  /**
   * Check if the current data source uses standard DICOMweb (no Volumes paths in imageIds).
   * When true, we need to call the backend API to resolve Volume paths.
   */
  needsPathResolution(viewport) {
    try {
      let imageIds = [];
      if (viewport && typeof viewport.getImageIds === 'function') {
      ...
      // Check the first imageId — if it doesn't contain /Volumes/, we need resolution
      if (imageIds.length > 0) {
        return !imageIds[0].includes('/Volumes/');
      }
    } catch (e) {/* ignore */}
    return false;
  }
   ...
   */
  async exportCombinedRedactionMetadata(viewportDetails) {
    const {
      viewportId,
      viewport,
      seriesInstanceUID,
    ...
    // Resolve Volume paths from backend if using standard DICOMweb data source
    let resolvedPaths = {};
    if (this.needsPathResolution(viewport)) {
      resolvedPaths = await this.resolveVolumePaths(studyInstanceUID, seriesInstanceUID);
      console.log(`Resolved ${Object.keys(resolvedPaths).length} volume paths from backend`);
    }

    // Get metadata for DICOM files
    const jsonMetadata = this.exportMetadata(viewport, resolvedPaths);
    ...
    // Get redaction areas
    const redactionAreas = this.exportRedactionAreas(viewportId, viewport, resolvedPaths);
    if (redactionAreas) {
    ...
    if (jsonMetadata.totalGlobalRedactions > 0 || jsonMetadata.totalMetadataRedactions > 0) {
      ...
      jsonMetadata.filesToEdit = Object.keys(jsonMetadata.frameRedactions);
    } else {
...
/* export default */ const services_RedactionService = (new RedactionService());
;// CONCATENATED MODULE: ../../../extensions/image-redactor/src/components/ExportSettings.tsx
/*
OHIF Image Redactor Extension Copyright (2025) Databricks, Inc.
Author: Emanuele Rinaldi <emanuele.rinaldi@databricks.com>
...
/* export default */ const components_ExportSettings = (ExportSettings);
;// CONCATENATED MODULE: ../../../extensions/image-redactor/src/components/RedactionAreasList.tsx
/*
OHIF Image Redactor Extension Copyright (2025) Databricks, Inc.
Author: Emanuele Rinaldi <emanuele.rinaldi@databricks.com>
...
/* export default */ const components_RedactionAreasList = (RedactionAreasList);
;// CONCATENATED MODULE: ../../../extensions/image-redactor/src/components/MetadataRedactionsList.tsx
/*
OHIF Image Redactor Extension Copyright (2025) Databricks, Inc.
Author: Emanuele Rinaldi <emanuele.rinaldi@databricks.com>
...
/* export default */ const components_MetadataRedactionsList = (MetadataRedactionsList);
;// CONCATENATED MODULE: ../../../extensions/image-redactor/src/components/RedactionSummary.tsx
/*
OHIF Image Redactor Extension Copyright (2025) Databricks, Inc.
Author: Emanuele Rinaldi <emanuele.rinaldi@databricks.com>
...
/* export default */ const components_RedactionSummary = (RedactionSummary);
;// CONCATENATED MODULE: ../../../extensions/image-redactor/src/components/AreaRedactionsSummary.tsx
/*
OHIF Image Redactor Extension Copyright (2025) Databricks, Inc.
Author: Emanuele Rinaldi <emanuele.rinaldi@databricks.com>
...
/* export default */ const components_AreaRedactionsSummary = (AreaRedactionsSummary);
;// CONCATENATED MODULE: ../../../extensions/image-redactor/src/components/MetadataRedactionsSummaryTable.tsx
/*
OHIF Image Redactor Extension Copyright (2025) Databricks, Inc.
Author: Emanuele Rinaldi <emanuele.rinaldi@databricks.com>
...
/* export default */ const components_MetadataRedactionsSummaryTable = (MetadataRedactionsSummaryTable);
;// CONCATENATED MODULE: ../../../extensions/image-redactor/src/components/PreviewPopup.tsx
/*
OHIF Image Redactor Extension Copyright (2025) Databricks, Inc.
Author: Emanuele Rinaldi <emanuele.rinaldi@databricks.com>
...
/* export default */ const components_PreviewPopup = (PreviewPopup);
;// CONCATENATED MODULE: ../../../extensions/image-redactor/src/components/ConfirmationDialog.tsx
/*
OHIF Image Redactor Extension Copyright (2025) Databricks, Inc.
Author: Emanuele Rinaldi <emanuele.rinaldi@databricks.com>
...
/* export default */ const components_ConfirmationDialog = (ConfirmationDialog);
;// CONCATENATED MODULE: ../../../extensions/image-redactor/src/components/RedactionPanel.tsx
/*
OHIF Image Redactor Extension Copyright (2025) Databricks, Inc.
Author: Emanuele Rinaldi <emanuele.rinaldi@databricks.com>
...
/* export default */ const components_RedactionPanel = (RedactionPanel);
;// CONCATENATED MODULE: ../../../extensions/image-redactor/src/getPanelModule.tsx
/*
OHIF Image Redactor Extension Copyright (2025) Databricks, Inc.
Author: Emanuele Rinaldi <emanuele.rinaldi@databricks.com>
...
// EXTERNAL MODULE: ../../../node_modules/react-window/dist/index.esm.js + 6 modules
var index_esm = __webpack_require__(42945);
...
;// CONCATENATED MODULE: ../../../extensions/image-redactor/src/components/DicomTagRedactorTable.tsx
...
const RowComponent = ({
  ...
  }, /*#__PURE__*/react.createElement(ui_next_src/* .Icons.ChevronDown */.FI1.ChevronDown, null)) : /*#__PURE__*/react.createElement("div", {
    className: "cursor-pointer p-1",
    onClick: handleToggle
  }, /*#__PURE__*/react.createElement(ui_next_src/* .Icons.ChevronRight */.FI1.ChevronRight, null))), /*#__PURE__*/react.createElement("div", {
...
/* export default */ const components_DicomTagRedactorTable = (/*#__PURE__*/react.memo(DicomTagRedactorTable));
;// CONCATENATED MODULE: ../../../extensions/image-redactor/src/models/MetadataShortcutTag.ts
/**
...
/* export default */ const models_MetadataShortcutTag = ((/* unused pure expression or super */ null && (MetadataShortcutTag)));
;// CONCATENATED MODULE: ../../../extensions/image-redactor/src/components/MetadataShortcuts.css
// extracted by css-extract-rspack-plugin

;// CONCATENATED MODULE: ../../../extensions/image-redactor/src/components/MetadataShortcuts.tsx
...
/* export default */ const components_MetadataShortcuts = (MetadataShortcuts);
;// CONCATENATED MODULE: ../../../extensions/image-redactor/src/components/DicomTagRedactor.css
// extracted by css-extract-rspack-plugin

;// CONCATENATED MODULE: ../../../extensions/image-redactor/src/components/DicomTagRedactor.tsx
...
const DicomTagRedactor = ({
  ...
  }, /*#__PURE__*/react.createElement(ui_next_src/* .InputFilter.SearchIcon */.zbB.SearchIcon, null), /*#__PURE__*/react.createElement(ui_next_src/* .InputFilter.Input */.zbB.Input, {
    placeholder: "Search metadata",
    className: "pl-9 pr-9"
  }), /*#__PURE__*/react.createElement(ui_next_src/* .InputFilter.ClearButton */.zbB.ClearButton, {
  ...
  while (stack.length > 0) {
    ...
    for (let i = index; i < tags.length; i++) {
      ...
      if (tagInfo.vr === 'SQ') {
        ...
        if (tagInfo.vr === 'xs') {
          try {
            const tag = dcmjs_es/* ["default"].data.Tag.fromPString */.Ay.data.Tag.fromPString(tagInfo.tag).toCleanString();
...
/* export default */ const components_DicomTagRedactor = (DicomTagRedactor);
;// CONCATENATED MODULE: ../../../extensions/image-redactor/src/components/DicomTagRedactorModal.tsx
/*
OHIF Image Redactor Extension Copyright (2025) Databricks, Inc.
Author: Emanuele Rinaldi <emanuele.rinaldi@databricks.com>
...
/* export default */ const components_DicomTagRedactorModal = (DicomTagRedactorModal);
;// CONCATENATED MODULE: ../../../extensions/image-redactor/src/getCommandsModule.ts
/*
OHIF Image Redactor Extension Copyright (2025) Databricks, Inc.
Author: Emanuele Rinaldi <emanuele.rinaldi@databricks.com>
...
;// CONCATENATED MODULE: ../../../extensions/image-redactor/src/init.ts
/*
OHIF Image Redactor Extension Copyright (2025) Databricks, Inc.
Author: Emanuele Rinaldi <emanuele.rinaldi@databricks.com>
...
;// CONCATENATED MODULE: ../../../extensions/image-redactor/src/index.tsx
/*
OHIF Image Redactor Extension Copyright (2025) Databricks, Inc.
Author: Emanuele Rinaldi <emanuele.rinaldi@databricks.com>
...
/* export default */ const image_redactor_src = ({
  id: id_id,
