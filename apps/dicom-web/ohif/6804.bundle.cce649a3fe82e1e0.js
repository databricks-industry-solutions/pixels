"use strict";
(globalThis["rspackChunk"] = globalThis["rspackChunk"] || []).push([[6804], {
41871(__unused_rspack_module, __webpack_exports__, __webpack_require__) {

// EXPORTS
__webpack_require__.d(__webpack_exports__, {
  "default": () => (/* binding */ nifti_segmentation_src)
});

// UNUSED EXPORTS: id

;// CONCATENATED MODULE: ../../../extensions/nifti-segmentation/package.json
var package_namespaceObject = JSON.parse('{"UU":"@ohif/extension-nifti-segmentation"}')
;// CONCATENATED MODULE: ../../../extensions/nifti-segmentation/src/id.ts
/*
...
/* export default */ const src_id = ((/* unused pure expression or super */ null && (id_id)));
// EXTERNAL MODULE: ../../../node_modules/react/index.js
var react = __webpack_require__(86326);
;// CONCATENATED MODULE: ../../../extensions/nifti-segmentation/src/components/NiftiOverlayPanel.css
// extracted by css-extract-rspack-plugin

// EXTERNAL MODULE: ../../../node_modules/axios/index.js
var axios = __webpack_require__(77738);
;// CONCATENATED MODULE: ../../../extensions/nifti-segmentation/src/services/NiftiOverlayClient.ts
/*
 * NiftiOverlayClient
 * ──────────────────
 *
...
class NiftiOverlayClient {
  ...
  async related(studyInstanceUID, seriesInstanceUID) {
    ...
    try {
      const response = await axios/* ["default"].get */.Ay.get(url, {
        headers,
        params: {
...
/* export default */ const services_NiftiOverlayClient = (NiftiOverlayClient);
...
// EXTERNAL MODULE: ../../../extensions/nifti-segmentation/node_modules/pako/dist/pako.esm.mjs
var pako_esm = __webpack_require__(15250);
;// CONCATENATED MODULE: ../../../extensions/nifti-segmentation/src/utils/affineUtils.ts
/*
 * affineUtils
 * ───────────
 *
...
;// CONCATENATED MODULE: ../../../extensions/nifti-segmentation/src/utils/NiftiReader.ts
/*
 * NiftiReader
 * ───────────
 *
...
/* export default */ const NiftiReader = (parseNifti);
...
;// CONCATENATED MODULE: ../../../extensions/nifti-segmentation/src/utils/injectLabelmap.ts
/*
 * injectLabelmap
 * ──────────────
 *
 ...
 *
 * Requires a series that reconstructs into a volume. OHIF v3.13's
 * `createLabelmapForDisplaySet` always registers a *stack*-backed labelmap
 * (`representationData.Labelmap.imageIds`, no `volumeId`), so this util
 * converts it to volume-backed before writing voxels — see
 * `resolveLabelmapVolume` for why that conversion is mandatory rather than a
 * convenience.
 *
...
async function injectLabelmap(options) {
  ...
  // Create the labelmap if it doesn't exist. Keyed on the segmentation rather
  // than on a cached volume: the volume is derived lazily below, so probing for
  // it here would re-create an already-registered segmentation.
  const existedBefore = Boolean(getLabelmapRepresentationData(segmentationId));
  if (!existedBefore) {
    try {
  ...
  // Resolve a volume-backed labelmap, converting from stack-backed if needed.
  const volume = await resolveLabelmapVolume(segmentationId, labelmapWaitMs, labelmapPollIntervalMs);
  if (!volume) {
...
/* export default */ const utils_injectLabelmap = (injectLabelmap);
...
/** Cornerstone-tools `representationData.Labelmap` for a segmentation, if registered. */
function getLabelmapRepresentationData(segmentationId) {
  try {
    return dist_esm.segmentation?.state?.getSegmentation(segmentationId)?.representationData?.Labelmap ?? null;
  } catch {
    return null;
  }
}
function getCachedLabelmapVolume(segmentationId) {
  const labelmap = getLabelmapRepresentationData(segmentationId);
  if (!labelmap?.volumeId) {
    return null;
  }
  try {
    return esm.cache.getVolume(labelmap.volumeId) ?? null;
  } catch {
    return null;
  }
}
...
/**
 * Return a volume-backed labelmap for `segmentationId`, converting the
 * stack-backed labelmap that OHIF registers into a volume when necessary.
 *
 * The conversion is load-bearing in two ways beyond just giving us a
 * `voxelManager` to write into:
 *
 *   - `segmentationService.getLabelmapVolume` only ever resolves via
 *     `representationData.Labelmap.volumeId`, which OHIF v3.13 never sets on
 *     creation. Without the conversion there is no volume to write to at all.
 *
 *   - The `SEGMENTATION_DATA_MODIFIED` event we fire after writing makes OHIF
 *     recompute segment statistics, and cornerstone-tools picks its code path
 *     off that same `volumeId`. The stack path (`calculateSegmentsStatisticsStack`)
 *     walks `segmentationInfo` and `imageInfo` in lockstep, but
 *     `prepareStackDataForWorker` only appends to `imageInfo` for slices whose
 *     referenced image is in the image cache — so the arrays desync and the
 *     compute worker dies on `imageInfo[i].scalarData`. Setting `volumeId`
 *     routes statistics through the volume path, which has no such bug.
 *
 * `ImageVolume`'s voxel manager writes through to the underlying stack
 * labelmap images, so the conversion does not fork the pixel data: stack and
 * volume viewports both render what we write here.
 */
async function resolveLabelmapVolume(segmentationId, totalWaitMs, pollIntervalMs) {
  const deadline = Date.now() + Math.max(0, totalWaitMs);

  // `loadSegmentationsForViewport` registers the representation asynchronously.
  let labelmap = getLabelmapRepresentationData(segmentationId);
  while (!labelmap?.volumeId && !labelmap?.imageIds?.length && Date.now() <= deadline) {
    await sleep(pollIntervalMs);
    labelmap = getLabelmapRepresentationData(segmentationId);
  }
  if (!labelmap) {
    return null;
  }
  const existing = getCachedLabelmapVolume(segmentationId);
  if (existing?.voxelManager) {
    return existing;
  }
  ...
  // A labelmap can only be volume-backed if its images reconstruct into one.
  // Bail with an actionable message instead of handing cornerstone a labelmap
  // whose geometry it will silently mis-derive.
  if (!esm.utilities.isValidVolume(imageIds)) {
    throw new InjectLabelmapError(`injectLabelmap: segmentationId='${segmentationId}' is backed by ${imageIds.length} ` + 'labelmap image(s) that do not reconstruct into a volume (irregular spacing, ' + 'mixed orientation, or a single instance). NIfTI overlays require a ' + 'reconstructable series displayed in a volume viewport.');
  }
  const convert = dist_esm.segmentation?.helpers?.convertStackToVolumeLabelmap;
  ...
  try {
    ...
    throw new InjectLabelmapError(`injectLabelmap: failed to convert segmentationId='${segmentationId}' to a volume labelmap: ${err?.message ?? String(err)}`, {
      cause: err
    });
  }
  const converted = getCachedLabelmapVolume(segmentationId);
  return converted?.voxelManager ? converted : null;
}
function sleep(ms) {
...
var ImageReslice = __webpack_require__(42700);
;// CONCATENATED MODULE: ../../../extensions/nifti-segmentation/src/utils/alignToDicomGrid.ts
/*
 * alignToDicomGrid
 * ────────────────
 *
...
/* export default */ const utils_alignToDicomGrid = (alignToDicomGrid);
...
;// CONCATENATED MODULE: ../../../extensions/nifti-segmentation/src/utils/flipToDicomGrid.ts
/*
 * flipToDicomGrid
 * ───────────────
 *
...
/* export default */ const flipToDicomGrid = ((/* unused pure expression or super */ null && (tryAxisAlignedRemap)));
...
;// CONCATENATED MODULE: ../../../extensions/nifti-segmentation/src/getCommandsModule.ts
/*
...
;// CONCATENATED MODULE: ../../../extensions/nifti-segmentation/src/components/NiftiOverlayPanel.tsx
/*
...
/* export default */ const components_NiftiOverlayPanel = (NiftiOverlayPanel);
...
;// CONCATENATED MODULE: ../../../extensions/nifti-segmentation/src/getPanelModule.tsx
...
;// CONCATENATED MODULE: ../../../extensions/nifti-segmentation/src/index.tsx
/*
...
/* export default */ const nifti_segmentation_src = (niftiSegmentationExtension);
