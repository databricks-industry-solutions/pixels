...
class TagExtractor(Transformer):
    ...
    def check_input_type(self, schema):
        ...
        if (field.dataType != t.StringType()):
            raise Exception('TagExtractor input type %s did not match input type StringType' % field.dataType)
    def _transform(self, df):
        self.check_input_type(df.schema)
        return (df
                .withColumn("relative_path", f.regexp_replace(self.inputCol, self.basePath+"(.*)$",r"$1"))
