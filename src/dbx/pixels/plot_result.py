...
class PlotResult:
    ...
    def _get_buttons(self) -> str:
        ...
        lst = [
            item for sublist in [y for y in map(lambda x: x[1], self._files)] for item in sublist
        ]
    ...
    def _get_rows(self):
        ...
        for item in self._files:
            ...
            if "FileStore" in file:
                ...
                row_src = (
                    row_src
                    + f'<div class="column {tag_str} content"><img src="{base_url}/{_src}"><p>{tag_str}</p></div>\n'
                )
        return row_src
